...
def login_and_get_token(wallet_address, private_key):
    ...
    response = requests.post(url_init, headers=HEADERS_COMMON, json=payload_init)
    ...
    response = requests.post(url_auth, headers=HEADERS_COMMON, json=payload_auth)
    response.raise_for_status()
...
if __name__ == "__main__":
    ...
    referral_code = "v7fuWXpPfXml"  # 替换为所需邀请码
